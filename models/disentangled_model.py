...
class DisentangledModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG.parameters(), self.netDepth.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))#同时更新netG和netDepth  其实netG和netDepth都是G
    ...
    def forward(self):
        ...
        self.fake_B = self.netG.forward(self.real_A)#GAN生成hazefreeimage fake_B
        ...
        self.fake_B2 = util.reverse_matting(self.real_A, self.depth)#根据透射图depth和默认大气光1.0被hazeimage恢复为hazefree image

        # reconstruct A based on optical model
        self.fake_A = util.synthesize_matting(self.fake_B, self.depth)#根据hazefreeimage和透射图depth生成haze image
    ...
    def backward_D_basic(self, netD, real, fake):
        ...
        # Combined loss
        loss_D = loss_D_fake + loss_D_real#fake和real都forward一遍 两个的loss叠加为总loss

        loss_D.backward()
        return loss_D
    
    def backward_D(self):
        fake_B = self.fake_B_pool.query(self.fake_B)#队列应该是为了批量计算损失
        self.loss_D = self.backward_D_basic(self.netD, self.real_B, self.fake_B)

    def backward_G(self):
        # First, G(A) should fake the discriminator
        pred_fake_B = self.netD.forward(self.fake_B)
        self.loss_G_B = self.criterionGAN(pred_fake_B, True)#GAN的对抗损失

        # Second, L1 loss for reconstruction
        self.loss_G_L1 = self.criterionL1(self.fake_A, self.real_A) * self.opt.lambda_A#groundtruth和predict的L1损失

        # Third, total variance loss
        self.loss_TV = self.criterionTV(self.depth) * self.opt.lambda_TV#透射图的方差大小作为损失，透射图越平滑总方差越小，为了平滑透射图，防止有块状效应

        self.loss_G = self.loss_G_L1 + self.loss_G_B + self.loss_TV#总损失
